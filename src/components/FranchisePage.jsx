...
import { FaStar, FaHeart, FaPlus, FaTrash, FaLink } from "react-icons/fa";
...
const FranchisePage = () => {
  ...
  const [linkDialog, setLinkDialog] = useState({ open: false, cardId: null, links: [] });
  // --- Persisted state ---
  const [gameCards, setGameCards] = useState(() => {
    try {
      const saved = localStorage.getItem(`franchise-gameCards-${franchiseName}`);
  ...
  const [gameRatings, setGameRatings] = useState(() => {
    try {
      const saved = localStorage.getItem(`franchise-gameRatings-${franchiseName}`);
      return saved ? JSON.parse(saved) : {};
    } catch {
      return {};
    }
  ...
  useEffect(() => {
    localStorage.setItem(`franchise-gameCards-${franchiseName}`,
      JSON.stringify(gameCards));
  }, [gameCards, franchiseName]);
  useEffect(() => {
    localStorage.setItem(`franchise-gameRatings-${franchiseName}`,
  ...
  const removeAllGameCards = () => {
    const confirmed = window.confirm("Are you sure you want to clear all games from this franchise?");
    if (confirmed) {
      setGameCards({});
      setGameRatings({});
    }
  ...
  }, [localCard?.text, localCard?.isFranchiseCard, reloadKey, localCard.id]);
  ...
  const isGameInFavorites = (game) => {
    return favoriteGames.some(favGame => Number(favGame.id) === Number(game.id));
  };

  const isGameInCurrentGames = (game) => {
    return currentGames.some(currentGame => Number(currentGame.id) === Number(game.id));
  };

  const isGameInNextGames = (game) => {
    return nextGames.some(nextGame => Number(nextGame.id) === Number(game.id));
  };

  const isGameInFinishedGames = (game) => {
    return finishedGames.some(finishedGame => Number(finishedGame.id) === Number(game.id));
  };
  ...
  }, [localRating, localCard?.id, franchiseName, cards, updateRating]);
  ...
  const isGameCardComplete = (card) => {
    return (
      ...
      card.releaseYear && card.releaseYear.trim() !== '' &&
      card.platforms && card.platforms.trim() !== '' &&
      card.image
    );
  };
      ...
      <div className="fixed top-4 right-4 sm:top-6 sm:right-6 md:right-8 lg:right-10 z-50 flex gap-2 sm:gap-3">
        ...
        <button
          onClick={() => {
            ...
            setGameCards(prev => ({ ...prev, [newCardId]: { title: "Title", image: null, releaseYear: "Empty", platforms: "Empty", links: [] }}));
            ...
            <div className="p-3 sm:p-4 overflow-y-auto max-h-[calc(85vh-80px)] sm:max-h-[calc(80vh-80px)] space-y-3">
              {Object.keys(gameCards).length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-gray-400">No games found in this collection. Add your own!</p>
                </div>
              ) : (
                Object.entries(gameCards).map(([cardId, card]) => (
                  <div key={cardId} className="flex items-center gap-3 bg-[#232323] p-3 rounded-lg">
                    <img
                      ...
                      <h3 className="text-white font-semibold text-xs sm:text-sm truncate">{card.title}</h3>
                    ...
                    <div className="flex gap-2 flex-shrink-0">
                      <button
                        ...
                        title={isGameInFavorites(card) ? "Already in favorites" : "Add to favorites"}
                        style={{ lineHeight: 0 }}
                        disabled={isGameInFavorites(card)}
                      >
                        <FaHeart className={isGameInFavorites(card) ? "text-red-500" : "text-white"} />
                      </button>
                      <button
                        onClick={() => {
                        ...
                        className="group p-1 bg-transparent border-none shadow-none rounded-full"
                        title={isGameInCurrentGames(card) ? "Already in current games" : "Add to current games"}
                        style={{ lineHeight: 0 }}
                        disabled={isGameInCurrentGames(card)}
                      >
                        <IoFlash className={isGameInCurrentGames(card) ? "text-yellow-400" : "text-white"} />
                      </button>
                      <button
                        onClick={() => {
                        ...
                        className="group p-1 bg-transparent border-none shadow-none rounded-full"
                        title={isGameInNextGames(card) ? "Already in next games" : "Add to next games"}
                        style={{ lineHeight: 0 }}
                        disabled={isGameInNextGames(card)}
                      >
                        <FaTerminal className={isGameInNextGames(card) ? "text-cyan-400" : "text-white"} />
                      </button>
                      <button
                        onClick={() => {
                        ...
                        className="group p-1 bg-transparent border-none shadow-none rounded-full"
                        title={isGameInFinishedGames(card) ? "Already in finished games" : "Add to finished games"}
                        style={{ lineHeight: 0 }}
                        disabled={isGameInFinishedGames(card)}
                      >
                        <IoMdCheckmark className={isGameInFinishedGames(card) ? "text-green-400" : "text-white"} />
                      </button>
                    </div>
                  </div>
                ))
          ...
          <div className="bg-[#1b1b1b] rounded-xl p-6 w-full max-w-lg shadow-2xl border border-gray-700 relative">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                onClick={() => setLinkDialog({ open: false, cardId: null, links: [] })}
                title="Close"
              >
            ...
            <div className="space-y-6 max-h-80 overflow-y-auto">
              {linkDialog.links.length === 0 ? (
                ...
                linkDialog.links.map((link, idx) => (
                  <div key={link.id || idx} className="bg-[#232323] rounded-lg p-4 border border-gray-700">
            ...
            <div className="flex justify-between items-center mt-6 pt-4 border-t border-gray-700">
              <button
                onClick={() => {
                  setLinkDialog(ld => ({ ...ld, links: [...ld.links, { id: Date.now(), text: '', url: '' }] }));
                }}
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => {
                    ...
                    newCards[linkDialog.cardId].links = linkDialog.links.filter(l => l.url);
        ...
        {Object.entries(gameCards).map(([cardId, card]) => (
          ...
          >
            ...
            <div className="flex flex-col justify-between flex-1 min-w-0 space-y-2">
              ...
              <div className="flex items-center gap-2 text-sm sm:text-base">
                <button
                  onClick={() => setLinkDialog({ open: true, cardId, links: card.links || [] })}
            ...
            <div className="flex flex-col justify-center self-center sm:self-auto flex-shrink-0 gap-2 pr-4">
              ...
              {parseFloat(gameRatings[cardId] || "0") > 0 ? (
                <button
                  ...
                  style={!isGameCardComplete(card) ? { opacity: 0.5, cursor: 'not-allowed' } : {}}
                >
